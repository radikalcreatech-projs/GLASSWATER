    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
      <section className="py-8 md:py-12 px-6 max-w-4xl mx-auto">
        <div className="text-center mb-6 md:mb-8">
          <h1 className="font-serif text-4xl sm:text-5xl font-bold text-navy mb-4">{faq.title}</h1>
          <p className="text-xl text-text-secondary">{faq.subtitle}</p>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {faq.items.map((faqItem, idx) => (
            <div key={idx} className="bg-bg-card border border-light-gray rounded-md overflow-hidden shadow-sm">
              ...
              >
                <p className="text-text-secondary">{faqItem.a}</p>
              </div>
            </div>
          ))}
        </div>
