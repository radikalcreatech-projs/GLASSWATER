...
export function RainCanvas() {
  ...
  useEffect(() => {
    ...
    // --- Offscreen canvas for static drops (drawn once, composited every frame) ---
    ...
    const draw = () => {
      if (paused) return;
      // Composite pre-rendered static layer in one call
      ctx.clearRect(0, 0, w, h);
      ctx.drawImage(offscreen, 0, 0);
      // Only update & draw the 25 dynamic drops each frame
