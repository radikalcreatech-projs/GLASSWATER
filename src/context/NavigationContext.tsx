...
export function NavigationProvider({ children }: { children: ReactNode }) {
  const [currentPage, setCurrentPage] = useState<Page>(() => {
    ...
    const pageOnly = raw.split('?')[0] as Page;
    return validPages.includes(pageOnly) ? pageOnly : 'home';
  });
  ...
  useEffect(() => {
    const onHashChange = () => {
      const raw = window.location.hash.replace('#', '');
      // Strip query params (e.g. "portal?code=GW-DEMO" → "portal")
      const pageOnly = raw.split('?')[0] as Page;
      if (validPages.includes(pageOnly)) {
        setCurrentPage(pageOnly);
        if (pageOnly !== 'post') setCurrentPostSlug(null);
      }
    };
